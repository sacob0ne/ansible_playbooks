...
'	date is older than [w] or [c] hours.  Also check if the file size is at least [z] bytes.
...
'Arguments:
'	directory path
'	warning age	(in hours)	warning failure if most recent file is [x] minutes old.
'	critical age (in hours)	critical failure if most recent file is [y] minutes old.
...
Err.Clear  
 ...
 'validate arguments
  ...
  argWarning = Abs(Wscript.Arguments.Item(1))	'Abs: absolute value required for comparison later, reported to minutes
...
fTopDate = Now
  ...
  'Determine age, in hours, of oldest file: DateDiff interval: yyyy-year, m-month, d-day, h=hour, n=minute, s=second

  fAge = Abs(DateDiff("h", fTopDate, Now)) 
  
  'Determine if file exceeds either warning or critical levels
'Wscript.echo("fAge: " & fAge & "  argCritical: " & argCritical & " fTopDate: " & fTopDate & " Now: " & Now)
  If fAge > argCritical Then
		'Wscript.echo("critical: " & fTop & " is " & fAge & " minutes old.")  
		'Wscript.echo("critical: " & fTop & " is older than " &argCritical & " hours" )  
		Wscript.echo("CRITICAL: there is almost one file older than " &argCritical & " hours in directory " &argFolder  )  
		Wscript.Quit(Exit_Critical)
  elseif fAge > argWarning Then
		'Wscript.echo("warning: " & fTop & " is " & fAge & " hours old.")
		'Wscript.echo("warning: " & fTop & " is older than " &argWarning & " hours" )  
		Wscript.echo("WARNING: there is almost one file older than " &argWarning & " hours in directory " &argFolder  )  
		Wscript.Quit(Exit_Warning)
  elseif fTopSize < argSize Then
		Wscript.echo("critcal: " & fTop & " is only " & fTopSize & " bytes.")
		Wscript.Quit(Exit_Critical)
  else
		'Wscript.echo("pass: " & fTop & " is " & fAge & " hours old.") 
		Wscript.echo("OK: no file is older than " &argCritical & " hours (CRITICAL) or " &argWarning & " hours (WARNING) in " &argFolder & " directory"  ) 
