...
' SERVICE QUERY --------------------------------------------------
    ...
    For Each objService in colServices
        ...
        If InStr(Trim(UCase(objService.DisplayName)),ProcName) > 0 OR InStr(Trim(UCase(objService.Name)),ProcName) Then
            ...
            If MyProcessID > 0 Then
                Set colProcess = objWMIService.ExecQuery("Select * from Win32_Process WHERE ParentProcessID=" & MyProcessID & " OR ProcessID=" & MyProcessID)
    ...
    If myObjItemState = "STOPPED" AND restartEnabled Then
        If Not patternMatch(UCase(restartString),".*RESTART FAILED.*",0) and Not patternMatch(UCase(restartString),".*RESTART_FAILED.*",0)Then
            If Not patternMatch(UCase(restartString),".*TRYING TO START.*",0) and Not patternMatch(UCase(restartString),".*TRYING_TO_START.*",0)Then
                If restartState(0) <> 0 AND ucase(restartState(1)) = "HARD" Then
                    ...
                    Next
                    StrOutput = "Action:TRYING_TO_START_ON " & StrOutput
                End If
            Else
                StrOutput = "Issue:RESTART_FAILED_ON " & StrOutput
            End If
        Else
            StrOutput = "Issue:RESTART_FAILED_ON " & StrOutput
        End If
