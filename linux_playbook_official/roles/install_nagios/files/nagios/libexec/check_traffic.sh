...
do
	case "$optname" in	"i") 	INT=$OPTARG
					;;
				"c")	CIRT=$OPTARG
					;;
				"w")	WARN=$OPTARG
					;;
				"h")	echo "Useage: check_traffic -i Interface -w warn -c cirt"
					exit
					;;
...
[ -z $WARN ] && WARN=104857600
[ -z $CIRT ] && CIRT=209715200
...
[ -e $FILE ] || >$FILE
if [ `cat $FILE | wc -c` -eq 0 ]
then
	echo -en `date +%s`"\t" 									 >$FILE
        echo -en `ifconfig $DEVICE | grep "RX bytes" | awk '{print $2}' | awk -F: '{print $NF}'`"\t" 	>>$FILE
        echo `/sbin/ifconfig $DEVICE | grep "RX bytes" | awk '{print $6}' | awk -F: '{print $NF}'`		>>$FILE
        echo "This is first run"
else
        New_Time=`date +%s`
        New_In=`/sbin/ifconfig $DEVICE | grep "RX bytes" | awk '{print $2}' | awk -F: '{print $NF}'`
        New_Out=`/sbin/ifconfig $DEVICE | grep "RX bytes" | awk '{print $6}' | awk -F: '{print $NF}'`
        ...
        if [ $Diff_In -gt $CIRT -o $Diff_In -eq $CIRT ]
	then
                echo -e "CRITICAL - Traffic for $INT | In=${Diff_In};${WARN};${CIRT};0;0; Out=${Diff_Out};${WARN};${CIRT};0;0"
                exit 2
        fi

        if [ $Diff_In -gt $WARN -o $Diff_In -eq $WARN ]
	then
                echo -e "WARNING - Traffic for $INT | In=${Diff_In};${WARN};${CIRT};0;0; Out=${Diff_Out};${WARN};${CIRT};0;0"
                exit 1
        fi

        if [ $Diff_In -lt $WARN ]
	then
                echo -e "OK - Traffic for $INT | In=${Diff_In};${WARN};${CIRT};0;0; Out=${Diff_Out};${WARN};${CIRT};0;0"
                exit 0
        fi
